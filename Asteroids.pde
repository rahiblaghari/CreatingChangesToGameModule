...
//if you wanna try to do what you did before, maybe use the if(state == finished) statement, but set the game to finished after the first run of code on Startcount, so it runs twice before it
//finishes, but only has to delay once (or maybe twice, but if so then set delay(0.500)
